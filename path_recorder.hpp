...
struct PathRecorder {
    ...
    void addPoint(CamInfo cam) {
        ...
        std::cout << "video time: " << original_path.back().t << std::endl;
    }

    void popPoint() {
        t_begin = fclock() - original_path.back().t;
    ...
    void smoothenPath() {
        ...
        std::cout << "smoothen path finished.\n";
    }
    ...
    void generateImage(Renderer &renderer) {
        ...
        std::cout << "generate image finished.\n";
    }
    void showPath(Renderer &renderer) {
        std::cout << "show path started.\n";
        for (int i = 0; i < path.size(); i++) {
            renderer.camera.pos = path[i].pos;
            renderer.camera.head = path[i].head;
            renderer.camera.front = path[i].front;
        ...
        std::cout << "show path finished.\n";
    }
    void storgePath(const char *name) {
        FILE *fp = fopen(name, "wb");
        int l = path.size();
        fwrite(&l, 1, 4, fp);
        for (int i = 0; i < path.size(); i++)
            fwrite(&path[i], 1, sizeof(CamInfo), fp);
        std::cout << "storge path finished.\n";
    }
    void readPath(const char *name) {
