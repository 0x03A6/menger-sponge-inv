...
inline void mainLoopPR(Renderer &renderer) {
    ...
    while (true) {
        ...
        if (GetAsyncKeyState('R')) {
            path_recorder.popPoint();
            renderer.camera.pos = path_recorder.original_path.back().cam.pos;
            renderer.camera.head = path_recorder.original_path.back().cam.head;
            renderer.camera.front = path_recorder.original_path.back().cam.front;
            renderer.camera.right = path_recorder.original_path.back().cam.right;
            calcInitBlock(renderer.camera.pos);
            renderer.camera.relative_pos = enterInitBlock(renderer.camera.pos);
            renderer.render();
            delay_ms(0);
